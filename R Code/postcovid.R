...
merge_postcovid <- bind_rows(as_tibble(arima_predict), as_tibble(ets_predict), as_tibble(prophet_predict))
merge_postcovid

# Ensemble model - average prediction of the three model classes
ensemble_postcovid <- merge_postcovid |>
  group_by(ds) |>
  ...
  inner_join(merge_postcovid |> filter(Method=="ARIMA") |> dplyr::select(ds, y), by="ds") |>
  relocate(ds, y, Method, Predict, Lower, Upper)
ensemble_postcovid

# Merge into one dataframe
asthma_postcovid <- bind_rows(as_tibble(merge_postcovid), as_tibble(ensemble_postcovid))
asthma_postcovid

# Save the predictions
dir <- "C:/Users/wiinu/OneDrive - cchmc/Documents/AHLS/Predictions/Postcovid Predictions/"
# saveRDS(asthma_postcovid, paste(dir, "postcovid_predict.rds", sep=""))
...
plot_postcovid <- asthma_postcovid |>
  ...
  ggtitle("Daily Asthma Hospitalization Forecast - During/Post-covid",
...
prophet.top <- asthma_postcovid |>
  ...
  slice_max(order_by=Predict, prop=p)
ensemble.top <- asthma_postcovid |>
  filter(Method=="Ensemble") |>
  slice_max(order_by=Predict, prop=p)
actual.top <- asthma_postcovid |>
  filter(Method=="ARIMA") |>
...
asthma_status <- asthma_postcovid |>
  ...
  mutate(Method=factor(Method, levels = c("ARIMA","ETS","Prophet","Ensemble"))) |>
  select(ds, y, actual.high, Method, Predict, model.high)
asthma_status

### Proportion of days where hospitalizations were classified as HIGH
# Actual HIGH days
asthma_status |>
  filter(Method=="ARIMA") |>
  count(actual.high) |>
...
asthma_status |>
  ...
  arrange(Method, desc(actual.high), desc(model.high))
